...
export class RopaComponent implements OnInit, AfterViewInit {
  tiposRopa: string[] = [];
  ...
  ngOnInit(): void {
    console.log('Ropa - ngOnInit iniciado');
    this.factorService.listarPorCategoria(3).subscribe((factores) => {
      this.factores = factores;
      console.log('Ropa - Factores cargados:', this.factores);
      this.tiposRopa = factores.map(f => f.nombre || f.nombreFactor || '').filter(Boolean);
      this.cargarConsumos();
    });
  }
  ...
  guardar(): void {
    ...
    const factor = this.factores.find(f => (f.nombre || f.nombreFactor) === this.formulario.tipoRopa) || this.factores[0];
    if (!factor) {
      console.error('Ropa - No hay factores disponibles');
      return;
    }
    ...
    const consumo: Consumo = {
      ...
      cantidad,
      detalles: [
        { clave: 'Tipo_Prenda', valor: this.formulario.tipoRopa }
      ]
    };
    console.log('Ropa - Payload a enviar:', consumo);
    this.guardando = true;
    const peticion = this.editandoId ? this.consumoService.editar(this.editandoId, consumo) : this.consumoService.registrar(consumo);
    peticion.subscribe({
      next: (res: any) => {
        console.log('Ropa - Respuesta del backend:', res);
        this.cargarConsumos();
        this.cancelar();
      },
      complete: () => (this.guardando = false),
      error: (error) => {
        console.error('Ropa - Error del backend:', error);
  ...
  editar(consumo: Consumo): void {
    ...
    this.formulario = {
      tipoRopa: consumo.factor?.nombre || this.buscarFactor(this.obtenerConsumoFactorId(consumo))?.nombre || `${this.detalle(consumo, 'Tipo_Prenda')}`,
  ...
  eliminar(id: any): void {
    ...
    if (cleanId && !isNaN(cleanId)) {
      console.log('Ropa - Eliminando consumo con ID:', cleanId);
      this.consumoService.eliminar(cleanId).subscribe({
        next: () => {
          console.log('Ropa - Consumo eliminado exitosamente');
          this.consumos = this.consumos.filter(c => this.obtenerConsumoId(c) !== cleanId);
          this.dataSource.data = [...this.consumos];
        },
        error: (error) => {
          console.error('Ropa - Error al eliminar consumo:', error);
        }
      });
    } else {
      console.error('Ropa - ID inválido para eliminar:', id);
    }
  }
  ...
  cancelar(): void {
    ...
    this.formulario = { tipoRopa: '', cantidad: '' };
  }
