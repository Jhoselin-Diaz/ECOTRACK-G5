...
export class HuellaCarbonoComponent implements OnInit {
  ...
  aplicarFiltro(): void {
    ...
    // Realizamos la llamada HTTP real para obtener la lista plana de consumos de usuarios comunes
    this.http.get<DetalleHuellaDTO[]>(`${this.apiUrl}/consumos/historico/filtrar`, { headers }).subscribe({
      next: (respuesta) => {
        ...
        agrupado.forEach(item => {
          ...
          listadoFinal.push({
            ...
            fechaCalculo: item.fechaCalculo, // 'MM/yyyy'
